...
export const getSlideIndexForNotMultipleItems = (
  currentIndex,
  items,
  slidesLength,
) => {
...
export const getSlideIndexForMultipleItems = (
  ...
  if (isNextSlideDisabled || (currentIndex < items && currentIndex !== 0))
    return dotsLength;
  if (currentIndex === 0) {
...
export const getFadeOutOffsetOnDotClick = (
  itemIndex,
  currentIndex,
  itemWidth,
) => {
...
export const getMaxSwipePosition = (items, itemWidth, slidesLength) =>
  (slidesLength + items) * itemWidth || 0;

export const getMinSwipePosition = (items, itemWidth) => items * itemWidth || 0;

export const recalculatePositionOnBeforeTouchEnd = (items, itemWidth) =>
  -getMinSwipePosition(items, itemWidth);
...
export const getMaxSwipeLimit = (maxSwipePosition, stagePadding = {}) => {
  ...
  const limit = paddingRight
    ? maxSwipePosition + paddingRight
    : maxSwipePosition;
  return limit || 0;
};

export const getSlideOffset = (itemWidth, offset = 250) =>
  Math.min(itemWidth / 2, offset) || 0;

export const getMinSwipeLimitIfNotInfinite = (items, itemWidth) =>
  items * itemWidth - getSlideOffset(itemWidth) || 0;
...
export const shouldRecalculateSwipePosition = (
  ...
  currentPosition >= 0 - minPosition ||
  Math.abs(currentPosition) >= maxPosition;

export const getMaxSwipeLimitIfNotInfinite = (slidesLength, itemWidth) =>
  slidesLength * itemWidth + getSlideOffset(itemWidth) || 0;
...
export const getItemIndexForDotNavigation = (
  ...
  const result = isTheLastIndex
    ? slidesLength - itemsLength
    : index * itemsLength;
  return result || 0;
};

export const isTheLastDotIndex = (index, infinite, dotsLength) =>
  infinite === false && index === dotsLength - 1;
